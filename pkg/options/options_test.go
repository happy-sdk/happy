...
func TestValidation(t *testing.T) {
	...
	validator := func(key string, val vars.Value) error {
		if val.String() == "invalid" {
			return fmt.Errorf("%w: invalid value", ErrOptionValidation)
		}
		return nil
	}
	testutils.NoError(t, opts.Add(NewOption("key1", "default", "desc", KindConfig, validator)))
	testutils.Error(t, opts.Add(NewOption("key1", "default", "desc", KindConfig, validator)))
	...
	testutils.NoError(t, opts.Add(NewOption("key2", "default", "desc", KindConfig, OptionValidatorNotEmpty)))
	err = opts.Set("key2", "")
...
func TestOptions_Seal(t *testing.T) {
	...
	t.Run("set default error", func(t *testing.T) {
		...
		specs := []Spec{
			NewOption("key1", "invalid", "desc1", KindConfig, func(key string, val vars.Value) error {
...
func TestOptions_MergeOptions(t *testing.T) {
	t.Run("valid merge", func(t *testing.T) {
		opts1, _ := New("dest", []Spec{
			NewOption("key1", "val1", "desc1", KindConfig, NoopValueValidator),
		})
		opts2, _ := New("src", []Spec{
	...
	t.Run("invalid merge", func(t *testing.T) {
		...
		opts2, _ := New("src", []Spec{
			NewOption("key2", vars.NilValue, "desc", KindConfig, NoopValueValidator),
		})
...
func TestOptions_Accepts(t *testing.T) {
	...
	opts, _ = New("test", []Spec{NewOption("key1", "val1", "desc1", KindConfig, NoopValueValidator)})
	if opts.Accepts("random") {
