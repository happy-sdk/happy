...
func TestMultipleEntries(t *testing.T) {
	...
	cron := newWithSeconds()
	if _, err := cron.AddFunc("0 0 0 1 1 ?", func() {}); err != nil {
		testutils.NoError(t, err)
	}
	...
	if _, err := cron.AddFunc("0 0 0 31 12 ?", func() {}); err != nil {
		testutils.NoError(t, err)
	}
	if _, err := cron.AddFunc("* * * * * ?", func() { wg.Done() }); err != nil {
		testutils.NoError(t, err)
...
func TestLocalTimezone(t *testing.T) {
	...
	cron := newWithSeconds()
	if _, err := cron.AddFunc(spec, func() { wg.Done() }); err != nil {
		testutils.NoError(t, err)
	}
	cron.Start()
	defer cron.Stop()
...
func TestJob(t *testing.T) {
	...
	cron := newWithSeconds()
	if _, err := cron.AddJob("0 0 0 30 Feb ?", testJob{wg, "job0"}); err != nil {
		testutils.NoError(t, err)
	}
	if _, err := cron.AddJob("0 0 0 1 1 ?", testJob{wg, "job1"}); err != nil {
		testutils.NoError(t, err)
	}
	job2, _ := cron.AddJob("* * * * * ?", testJob{wg, "job2"})
	if _, err := cron.AddJob("1 0 0 1 1 ?", testJob{wg, "job3"}); err != nil {
		testutils.NoError(t, err)
	}
...
func TestStopAndWait(t *testing.T) {
	...
	t.Run("a couple fast jobs added, still returns immediately", func(t *testing.T) {
		cron := newWithSeconds()
		if _, err := cron.AddFunc("* * * * * *", func() {}); err != nil {
			testutils.NoError(t, err)
		}
		cron.Start()
		if _, err := cron.AddFunc("* * * * * *", func() {}); err != nil {
	...
	t.Run("a couple fast jobs and a slow job added, waits for slow job", func(t *testing.T) {
		...
		cron.Start()
		if _, err := cron.AddFunc("* * * * * *", func() { time.Sleep(2 * time.Second) }); err != nil {
			testutils.NoError(t, err)
		}
		if _, err := cron.AddFunc("* * * * * *", func() {}); err != nil {
			testutils.NoError(t, err)
