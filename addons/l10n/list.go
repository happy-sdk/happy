...
func l10nList() *command.Command {
	...
	cmd.Do(func(sess *session.Context, args action.Args) error {
		...
		if !includeDeps {
			...
			// Process app and dependency entries the same way, respecting the
			// same --missing flag for both: a dependency section that only
			// ever shows entries with something missing (regardless of
			// --missing) would make a fully-translated dependency
			...
			depKeyStatuses := processEntriesForDisplay(depEntries, translatableLangs, targetLang, fallbackLang, missingOnly)
