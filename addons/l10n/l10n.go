...
import (
	...
	_ "github.com/happy-sdk/happy/addons/l10n/l10n" // registers this addon's own bundle - see that package's doc comment
...
const l10np = "com.github.happy-sdk.happy.addons.l10n"
...
func Command(cnf CommandConfig) *command.Command {
	...
	if !cnf.WithoutGenerate {
		subcmds = append(subcmds, l10nGenerate())
	}
...
// Addon returns the l10n addon: registering it with app.WithAddon(s) adds
// the l10n command (list/report/translate) to the application's CLI. Pass
// a CommandConfig (e.g. a customized copy of DefaultCommandConfig()) to
// override the command's name, category, description, or which
// subcommands it exposes; omit it to use DefaultCommandConfig() as-is.
func Addon(cnf ...CommandConfig) *addon.Addon {
	c := DefaultCommandConfig()
	if len(cnf) > 0 {
		c = cnf[0]
	}
	return addon.New("L10n").
		WithConfig(addon.Config{
			Slug: "l10n",
		}).
		ProvideCommands(Command(c))
}
