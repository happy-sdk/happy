	...
	cmd.Do(func(sess *session.Context, args action.Args) error {
		...
		if !includeDeps {
			...
			for _, lang := range languages {
				...
				filteredReport := filterReportByDependencies(sess, report, deps, false)
			...
			if len(languages) > 0 {
				...
				filteredFirstReport := filterReportByDependencies(sess, firstReport, deps, false)
			...
			// Dependency translations section - shown whenever --with-deps is
			// passed, whether or not anything is missing: the whole point of
			// asking to see dependency coverage is confirming it's complete,
			// so a silently-omitted table here would be indistinguishable
			// from "never checked".
			...
			for _, lang := range languages {
				...
				appReport := filterReportByDependencies(sess, report, deps, false)
				...
				// Add dependency translation statistics row
			...
			// immediately after the application translations table, before
			// the dependency translations section begins.
			if len(languages) > 0 {
				firstReport := i18n.GetTranslationReport(languages[0])
				filteredFirstAppReport := filterReportByDependencies(sess, firstReport, deps, false)

				if len(filteredFirstAppReport.RootKeys) > 0 {
					perRootTable := textfmt.NewTable(
					...
					for _, rootKey := range filteredFirstAppReport.RootKeys {
						stats := filteredFirstAppReport.PerRootKey[rootKey]
						perRootTable.AddRow(
							rootKey,
							fmt.Sprintf("%d", stats.Total),
							fmt.Sprintf("%d", stats.Translated),
			...
			// Add per-root-key statistics for dependency translations at the
			// very end of the report, after the dependency translations table.
			if len(languages) > 0 {
				firstReport := i18n.GetTranslationReport(languages[0])
				filteredFirstDepReport := filterReportByDependencies(sess, firstReport, deps, true)

				if len(filteredFirstDepReport.RootKeys) > 0 {
					perRootTable := textfmt.NewTable(
						textfmt.TableTitle("Per Root Key Statistics (Dependencies)"),
						textfmt.TableWithHeader(),
					)
					perRootTable.AddRow("Root Key", "Total", "Translated", "Missing", "Percentage")

					// Use first language's report for root key statistics breakdown
					for _, rootKey := range filteredFirstDepReport.RootKeys {
						stats := filteredFirstDepReport.PerRootKey[rootKey]
						perRootTable.AddRow(
							rootKey,
							fmt.Sprintf("%d", stats.Total),
							fmt.Sprintf("%d", stats.Translated),
...
// filterReportByDependencies filters a translation report down to just the
// application's own keys (keepDependencyKeys false) or just dependency keys
// (keepDependencyKeys true).
func filterReportByDependencies(sess *session.Context, report i18n.TranslationReport, deps map[string]bool, keepDependencyKeys bool) i18n.TranslationReport {
