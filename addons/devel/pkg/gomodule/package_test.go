...
func TestPackage_SyncGoVersion(t *testing.T) {
	t.Run("no-op when already in sync", func(t *testing.T) {
		...
		if err := p.SyncGoVersion("1.25.0"); err != nil {
	...
	t.Run("updates and bumps patch when out of sync", func(t *testing.T) {
		p := newTestPackage(t, "1.25.0", "pkg/mod/", "pkg/mod/v1.2.3", false)
		if err := p.SyncGoVersion("1.26.4"); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if !p.NeedsRelease {
		...
		if p.NextReleaseTag != "pkg/mod/v1.2.4" {
			t.Errorf("expected patch-bumped next release tag pkg/mod/v1.2.4, got %s", p.NextReleaseTag)
		}
	})
	...
	t.Run("no-op when root version is unknown", func(t *testing.T) {
		p := newTestPackage(t, "1.25.0", "pkg/mod/", "pkg/mod/v1.2.3", false)
		if err := p.SyncGoVersion(""); err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if p.NeedsRelease {
