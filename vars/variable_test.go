...
func TestVariableBool(t *testing.T) {
	for _, test := range boolTests {
		v := New(test.key, test.in)
		if v.Bool() != test.want {
			t.Errorf("TestBool(%s): expected %t got %t", test.in, test.want, v.Bool())
			continue
		}

		vt, err := NewTyped(test.key, test.in, TypeBool)
		if v.Bool() != vt.Bool() {
...
func TestVariableFloat32(t *testing.T) {
	for _, test := range float32Tests {
		...
		vt, err := NewTyped(test.key, test.in, TypeFloat32)
...
func TestVariableFloat64(t *testing.T) {
	for _, test := range float64Tests {
		...
		vt, err := NewTyped(test.key, test.in, TypeFloat64)
		if vt.String() != test.wantStr {
...
func TestVariableComplex64(t *testing.T) {
	for _, test := range complex64Tests {
		...
		vt, err := NewTyped(test.key, test.in, TypeComplex64)
		if vt.String() != test.wantStr {
...
func TestVariableComplex128(t *testing.T) {
	for _, test := range complex128Tests {
		...
		vt, err := NewTyped(test.key, test.in, TypeComplex128)
		if vt.String() != test.wantStr {
...
func TestVariableInt(t *testing.T) {
	for _, test := range intTests {
		t.Run(fmt.Sprintf("%s(int): %q", test.key, test.val), func(t *testing.T) {
			...
			vu := New(test.key, test.val)
			if vu.Int() != test.int {
				t.Errorf("untyped expected %d got %d", test.int, vu.Int())
			}

			vp, _ := Parse(test.key, test.val)
			if vp.Int() != test.int {
		...
		t.Run(fmt.Sprintf("%s(int8): %q", test.key, test.val), func(t *testing.T) {
			v, err := NewTyped(test.key, test.val, TypeInt8)
		...
		t.Run(fmt.Sprintf("%s(int16): %q", test.key, test.val), func(t *testing.T) {
			v, err := NewTyped(test.key, test.val, TypeInt16)
		...
		t.Run(fmt.Sprintf("%s(int32): %q", test.key, test.val), func(t *testing.T) {
			v, err := NewTyped(test.key, test.val, TypeInt32)
		...
		t.Run(fmt.Sprintf("%s(int64): %q", test.key, test.val), func(t *testing.T) {
			v, err := NewTyped(test.key, test.val, TypeInt64)
...
// func (v Variable) UInt?n() uint?n
func TestVariableUint(t *testing.T) {
	for _, test := range uintTests {
		t.Run(fmt.Sprintf("%s(uint): %q", test.key, test.val), func(t *testing.T) {
			v, err := NewTyped(test.key, test.val, TypeUint)
		...
		t.Run(fmt.Sprintf("%s(uint8): %q", test.key, test.val), func(t *testing.T) {
			v, err := NewTyped(test.key, test.val, TypeUint8)
		...
		t.Run(fmt.Sprintf("%s(uint16): %q", test.key, test.val), func(t *testing.T) {
			v, err := NewTyped(test.key, test.val, TypeUint16)
		...
		t.Run(fmt.Sprintf("%s(uint32): %q", test.key, test.val), func(t *testing.T) {
			v, err := NewTyped(test.key, test.val, TypeUint32)
		...
		t.Run(fmt.Sprintf("%s(uint64): %q", test.key, test.val), func(t *testing.T) {
			v, err := NewTyped(test.key, test.val, TypeUint64)
...
func TestVariableUintptr(t *testing.T) {
	tests := []struct {
		key  string
		val  string
		want uintptr
	...
	for _, test := range tests {
		v, err := NewTyped(test.key, test.val, TypeUintptr)
...
func TestVariableStringParse(t *testing.T) {
...
func TestVariableStringNew(t *testing.T) {
	for _, test := range stringTests {
		v := New(test.key, test.val)
		if v.Key() != test.key {
			t.Errorf("New %q, %q: expected key %q got %q", test.key, test.val, test.key, v.Key())
			continue
		}
		if v.String() != test.val {
...
func TestVariableStringType(t *testing.T) {
	for _, test := range stringTests {
		v, _ := NewTyped(test.key, test.val, TypeString)
		if v.Key() != test.key {
			t.Errorf("New %q, %q: expected key %q got %q", test.key, test.val, test.key, v.Key())
			continue
		}
