...
func New(out io.Writer, options ...happy.OptionSetFunc) *Logger {
	l := &Logger{
		out:            out,
...
func (l *Logger) outputWithDuration(lvl happy.LogPriority, ltype string, calldepth int, s string, ts time.Time, fg, bg color) error {
	now := time.Now() // get this early.
	var file string
	var line int
	l.mu.Lock()
