...
func Bexp(name string, value string, usage string, aliases ...string) (flag *BexpFlag, err error) {
	...
	flag.defval, err = vars.NewVariableAs(name, value, false, vars.KindString)
	if err != nil {
		return nil, err
	}
	flag.usage = usage
	flag.val = []string{}
	flag.variable, err = vars.NewVariableAs(name, value, false, vars.KindString)
	return flag, err
}
...
func (f *BexpFlag) Parse(args []string) (ok bool, err error) {
	...
	f.variable, err = vars.NewVariable(f.name, strings.Join(f.val, "|"), false)
	return ok, err
}
...
func (f *BexpFlag) Unset() {
	...
	f.variable, _ = vars.NewVariable(f.name, strings.Join(defaults, "|"), false)
	f.isPresent = false
	f.val = defaults
}
