...
func Option(name string, value []string, opts []string, usage string, aliases ...string) (flag *OptionFlag, err error) {
	...
	flag.defval, err = vars.NewVariableAs(name, strings.Join(value, "|"), true, vars.KindString)
	if err != nil {
		return nil, err
	}
	for _, o := range opts {
	...
	flag.variable, err = vars.NewVariableAs(name, strings.Join(value, "|"), true, vars.KindString)
	return flag, err
}
...
func (f *OptionFlag) Parse(args []string) (ok bool, err error) {
	...
	if !f.defval.Empty() {
		...
		for _, dd := range defval {
			v, err := vars.NewVariableAs(f.name, dd, false, vars.KindString)
			if err != nil {
				return false, err
			}
			opts = append(opts, v)
	...
	if len(opts) > 0 {
		...
		f.val = str
		f.parsed = true
		v, err := vars.NewVariableAs(f.name, strings.Join(str, "|"), false, vars.KindString)
		if err != nil {
			return false, err
		}
		f.variable = v
