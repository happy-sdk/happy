...
import (
	...
	"github.com/happy-sdk/happy/pkg/platform/daemon"
...
func convertToSystemService(main *Main, settingsb *settings.Blueprint) error {
	appName, err := settingsb.GetSpec("app.name")
	if err != nil {
		return err
	}
	appDesc, err := settingsb.GetSpec("app.description")
	if err != nil {
		return err
	}
	d, err := daemon.New(daemon.Settings{
		Name:        settings.String(appName.Value + " Daemon"),
		Description: settings.String(appDesc.Value),
		Slug:        settings.String(main.slug),
		Kind:        daemon.KindUser,
	})
	if err != nil {
		return err
	}
	...
	syssvc := &happyDaemon{
		daemon:             d,
		beforeAction:       main.root.beforeAction,
		doAction:           main.root.doAction,
		afterSuccessAction: main.root.afterSuccessAction,
		afterFailureAction: main.root.afterFailureAction,
		afterAlwaysAction:  main.root.afterAlwaysAction,
	}

	beforeAlways := main.beforeAlways
	main.beforeAlways = func(sess *Session, flags Args) error {
		d.SetPIDFile(main.instance.DaemonPIDFile())
		if beforeAlways != nil {
			return beforeAlways(sess, flags)
		}
		return nil
	}
	main.root.beforeAction = nil
	main.root.doAction = nil
	main.root.afterSuccessAction = nil
	main.root.afterFailureAction = nil
	main.root.afterAlwaysAction = nil

	main.root.AddSubCommand(syssvc.Command(main))
	return nil
}

type happyDaemon struct {
...
func (s *happyDaemon) Command(main *Main) *Command {
	cmd := NewCommand("service",
		Option("description", "System service management"),
		Option("category", "service"),
	)

	cmd.AddSubCommand(NewCommand("install",
		Option("description", `Installs the application as a system service,
    If the application system service is already installed, it will be reinstalled.`),
	).Do(func(sess *Session, args Args) error {
		if err := s.daemon.Install(); err != nil {
			return err
		}
		sess.Log().Ok("service installed")
		return nil
	}))

	cmd.AddSubCommand(NewCommand("start",
		Option("description", `Starts the application as an OS-specific background service.
    Automatically installs it as a system startup service if it's not installed already.`),
	).Do(func(sess *Session, args Args) error {
		return s.daemon.Start()
	}))

	cmd.AddSubCommand(NewCommand("stop",
		Option("description", "Stops the application if it's running as a background service"),
	).Do(func(sess *Session, args Args) error {
	...
	{
		run := NewCommand("run",
			Option("description", "Starts the application as a foreground process"),
		)

		run.beforeAction = func(sess *Session, args Args) error {
			if err := main.instance.Daemonize(); err != nil {
				return err
			}
			if s.beforeAction != nil {
				return s.beforeAction(sess, args)
			}
			return nil
		}
		run.doAction = s.doAction
		run.afterSuccessAction = s.afterSuccessAction
		run.afterFailureAction = s.afterFailureAction
		run.afterAlwaysAction = s.afterAlwaysAction
		cmd.AddSubCommand(run)
	}

	cmd.AddSubCommand(NewCommand("status",
		Option("description", "Shows the current status of the application service"),
	).Do(func(sess *Session, args Args) error {
		...
		switch status {
		case daemon.StatusActive:
			sess.Log().Ok("service is active")
		case daemon.StatusInactive:
			sess.Log().Warn("service is inactive")
		case daemon.StatusFailed:
			sess.Log().Error("service failed")
		case daemon.StatusReloading:
			sess.Log().Notice("service is reloading")
		case daemon.StatusActivating:
			sess.Log().Notice("service is activating")
		case daemon.StatusDeactivating:
			sess.Log().Notice("service is deactivating")
		default:
			sess.Log().Error("unknown service status")
		}

		sess.Log().Println("service status", slog.Any("status", status))
		return nil
	}))

	cmd.AddSubCommand(NewCommand("enable",
		Option("description", "Enables the application to start as a background service on system startup"),
	).Do(func(sess *Session, args Args) error {
		return s.daemon.Enable()
	}))

	cmd.AddSubCommand(NewCommand("disable",
		Option("description", "Disables the application from starting as a background service on system startup"),
	).Do(func(sess *Session, args Args) error {
		return s.daemon.Disable()
	}))

	cmd.AddSubCommand(NewCommand("logs",
		Option("description", "Shows the logs of the application service"),
	).Do(func(sess *Session, args Args) error {
		return s.daemon.Logs()
	}))

	cmd.AddSubCommand(NewCommand("uninstall",
		Option("description", "Uninstalls the application as a system service"),
	).Do(func(sess *Session, args Args) error {
		return s.daemon.Uninstall()
	}))

	cmd.AddSubCommand(NewCommand("restart",
		Option("description", "Restarts the application service"),
	).Do(func(sess *Session, args Args) error {
		return s.daemon.Restart()
	}))
